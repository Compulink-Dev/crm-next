...
const Sidebar: React.FC<{
  ...
  const checkPermission = (item: MenuItem) => {
    if (item.roles && session?.user?.role) {
      console.log(
        ...
        "User Role: ",
        session.user.role
      ); // Log roles required for the item
  ...
  return (
    <div
      className={`${
        showSide
          ? "w-64 h-full fixed p-4 bg-slate-900 text-white md:flex flex-col z-50 overflow-y-scroll scrollbar-hide"
          : "w-64 h-full fixed p-4 bg-slate-900 text-white md:flex flex-col hidden z-50 overflow-y-scroll scrollbar-hide"
      }`}
    >
      <div className="flex items-center justify-between">
        <div className="flex gap-2 items-center p-4">
          <Webhook />
          <p className="">CRM Dashboard</p>
        </div>
        ...
        {menuItems.map((item) => (
          <li key={item.title} className="group p-2 hover:text-slate-700 ">
                    ...
                    {item.children.map(
                      (subItem) =>
                        checkPermission(subItem) ? (
                          <li className="text-sm" key={subItem.title}>
                            <Link
            ...
            checkPermission(item) ? (
              <Link className="block rounded-md" href={item.path || "#"}>
                <span className="flex items-center ">
                  {item.icon && (
                    <item.icon className="w-4 h-4 mr-2 text-gray-400" />
                  )}
